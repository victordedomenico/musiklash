...
type Tab = "game" | "collection" | "entity";
...
export default function IndiePicker({ size, selected, onChange, freeMode = false }: Props) {
  ...
  const [entityGames, setEntityGames] = useState<ContentItem[]>([]);
  ...
  const { results: entityResults, loading: entityLoading } = useDebouncedSearch(
    "/api/content/search?kind=entities",
    query,
    tab === "entity" && !openedEntity,
  );
  ...
  const openEntity = async (entity: ContentEntity) => {
    ...
    try {
      ...
      const json = await res.json();
      setEntityGames(json.items ?? json.results ?? []);
    } catch (e) {
      console.error(e);
    } finally {
      setDrillLoading(false);
  ...
  const tabs: { key: Tab; label: string; icon: typeof Gamepad2 }[] = [
    ...
    { key: "collection", label: "Tags", icon: Layers },
    { key: "entity", label: "Jams & devs", icon: Users },
  ];

  const entityItems = entityResults as unknown as ContentEntity[];
  ...
  return (
    <div className="space-y-4">
      ...
      {!openedCollection && !openedEntity && (
        <div className="relative">
          ...
          <input
            ...
            placeholder={
              tab === "game"
                ? "Rechercher un jeu indé…"
                : tab === "collection"
                  ? "Rechercher un tag (roguelike, pixel…)…"
                  : "Rechercher une jam ou un studio…"
            }
      ...
      <div className="min-h-[200px] space-y-2">
          ...
          (collectionResults as unknown as ContentCollection[]).map((col) => (
            ...
            >
              ...
              <div>
                ...
                <p className="text-xs text-[color:var(--muted)]">Jeux du tag</p>
              </div>
            </button>
          ))}
            ...
            >
              {entity.pictureUrl ? (
                ...
                <div className="w-12 h-12 rounded-full bg-[color:var(--surface-2)] flex items-center justify-center">
                  {entity.id.startsWith("jam-") ? (
                    <Sparkles size={20} className="text-[color:var(--muted)]" />
                  ) : (
                    <Users size={20} className="text-[color:var(--muted)]" />
                  )}
                </div>
              )}
              <div>
                ...
                <p className="text-xs text-[color:var(--muted)]">
                  {entity.id.startsWith("jam-") ? "Jeux de la jam" : "Jeux du studio"}
                </p>
              </div>
            </button>
          ))}
        ...
        {(gameLoading || collectionLoading || entityLoading) && (
