...
import ComicPicker, { type SelectedItem } from "@/components/ComicPicker";
...
export default function CreateBracketForm() {
  ...
  const onSubmit: React.FormEventHandler<HTMLFormElement> = (e) => {
    ...
    if (selected.length < 3) {
      setError("Sélectionne au moins 3 numéros.");
      return;
    }
  ...
  return (
    <form onSubmit={onSubmit} className="space-y-6">
      <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
        <div>
          ...
          <Input
            ...
            placeholder="Ex. Meilleurs numéros Batman"
          />
        </div>
        <div>
      ...
      <ComicPicker size={size} selected={selected} onChange={setSelected} />
