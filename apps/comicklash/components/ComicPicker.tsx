...
import { Search, Plus, X, ChevronLeft, Check, BookOpen, Layers, User } from "lucide-react";
...
function itemToSelected(item: ContentItem): SelectedItem {
  return {
    ...
    preview_url: null,
    source: "comicvine",
    metadata: { itemKind: item.metadata?.itemKind ?? "issue", ...item.metadata },
  };
}

export default function ComicPicker({ size, selected, onChange, freeMode = false }: Props) {
  const [tab, setTab] = useState<Tab>("issue");
  const [query, setQuery] = useState("");
  const [openedArc, setOpenedArc] = useState<ContentCollection | null>(null);
  const [openedCharacter, setOpenedCharacter] = useState<ContentEntity | null>(null);
  const [arcIssues, setArcIssues] = useState<ContentItem[]>([]);
  const [characterIssues, setCharacterIssues] = useState<ContentItem[]>([]);
  const [drillLoading, setDrillLoading] = useState(false);
  const inputRef = useRef<HTMLInputElement>(null);

  const { results: issueResults, loading: issueLoading } = useDebouncedSearch(
  ...
  const openArc = async (arc: ContentCollection) => {
    setOpenedArc(arc);
    setOpenedCharacter(null);
    setDrillLoading(true);
    setArcIssues([]);
    try {
      const res = await fetch(`/api/content/collection/${arc.id}/items`);
      const json = await res.json();
      setArcIssues(json.items ?? json.results ?? []);
    } catch (e) {
      console.error(e);
    } finally {
  ...
  const openCharacter = async (character: ContentEntity) => {
    setOpenedCharacter(character);
    setOpenedArc(null);
    setDrillLoading(true);
    setCharacterIssues([]);
    try {
      const res = await fetch(`/api/content/entity/${character.id}/items?limit=50`);
      const json = await res.json();
      setCharacterIssues(json.items ?? json.results ?? []);
    } catch (e) {
      console.error(e);
    } finally {
      setDrillLoading(false);
  ...
  const tabs: { key: Tab; label: string; icon: typeof BookOpen }[] = [
    { key: "issue", label: "Numéros", icon: BookOpen },
    { key: "arc", label: "Séries & arcs", icon: Layers },
  ...
  return (
    <div className="space-y-4">
          ...
          <input
            ...
            placeholder={
              tab === "issue"
                ...
                : tab === "arc"
                  ? "Rechercher une série ou un arc…"
      ...
      <div className="min-h-[200px] space-y-2">
        ...
        {tab === "arc" && !openedArc &&
          (arcResults as unknown as ContentCollection[]).map((arc) => (
            <button
              key={arc.id}
              type="button"
              ...
              onClick={() => void openArc(arc)}
            >
              {arc.coverUrl ? (
                <Image src={arc.coverUrl} alt="" width={48} height={72} className="rounded-md object-cover" />
              ) : (
                <div className="w-12 h-[72px] rounded-md bg-[color:var(--surface-2)] flex items-center justify-center">
                  <Layers size={20} className="text-[color:var(--muted)]" />
                </div>
              )}
              <div>
                <p className="font-semibold">{arc.title}</p>
                <p className="text-xs text-[color:var(--muted)]">Voir les numéros</p>
              </div>
            </button>
          ))}
        ...
        {tab === "arc" && openedArc && (
          <>
            <p className="text-sm font-medium text-[color:var(--muted)]">{openedArc.title}</p>
        ...
        {tab === "character" && !openedCharacter &&
          (characterResults as unknown as ContentEntity[]).map((character) => (
            <button
              key={character.id}
              type="button"
              className="w-full flex items-center gap-3 rounded-xl border border-[color:var(--border)] p-3 text-left hover:bg-[color:var(--surface-hover)]"
            ...
            >
              ...
              <div>
                ...
                <p className="text-xs text-[color:var(--muted)]">Apparitions</p>
              </div>
            </button>
          ))}
        ...
        {tab === "character" && openedCharacter && (
          <>
            <p className="text-sm font-medium">{openedCharacter.name}</p>
            {drillLoading && <p className="text-sm text-[color:var(--muted)]">Chargement…</p>}
            {characterIssues.map((item) => (
              <ResultRow
                key={item.id}
                title={item.title}
                subtitle={item.subtitle}
        ...
        {(issueLoading || arcLoading || characterLoading) && (
