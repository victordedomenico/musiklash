...
type Tab = "match" | "tournament" | "team";
...
function itemToSelected(item: ContentItem): SelectedItem {
  return {
    ...
    preview_url: null,
    source: "api-sports",
    metadata: { itemKind: item.metadata?.itemKind ?? "fight", ...item.metadata },
  };
}

export default function EsportPicker({ size, selected, onChange, freeMode = false }: Props) {
  ...
  const { results: fighterItemResults, loading: fighterItemLoading } = useDebouncedSearch(
    "/api/content/search?kind=fighter",
    query,
    tab === "team" && !openedEntity,
  );
  ...
  const tabs: { key: Tab; label: string; icon: typeof Swords }[] = [
    { key: "match", label: "Combats", icon: Swords },
    { key: "tournament", label: "Styles & events", icon: Trophy },
    { key: "team", label: "Fighters & orgs", icon: Users },
  ];
  ...
  return (
    <div className="space-y-4">
          ...
          <input
            ...
            placeholder={
              tab === "match"
                ...
                : tab === "tournament"
                  ? "Rechercher un style ou un event…"
                  : "Rechercher un fighter ou une org…"
            }
      ...
      <div className="min-h-[200px] space-y-2">
          ...
          (tournamentResults as unknown as ContentCollection[]).map((col) => (
            ...
            >
              ...
              <div>
                ...
                <p className="text-xs text-[color:var(--muted)]">Voir les combats</p>
              </div>
            </button>
          ))}
        ...
        {tab === "team" && !openedEntity && (
          <>
            {fighterItemResults.map((item) => (
              <ResultRow
                key={item.id}
                title={item.title}
                subtitle={item.subtitle ?? "Fighter"}
                coverUrl={item.coverUrl}
                selected={isSelected(item.id)}
                onAdd={() => addItem(item)}
              />
            ...
            >
              ...
              <div>
                ...
                <p className="text-xs text-[color:var(--muted)]">
                  {entity.id.startsWith("org-") ? "Combats de l'organisation" : "Combats du fighter"}
                </p>
              </div>
            </button>
          ))}
        ...
        {(matchLoading || tournamentLoading || teamLoading || fighterItemLoading) && (
