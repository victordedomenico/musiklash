...
import { getTopCountries } from "@/lib/top-countries";
import { absoluteUrl, buildPageMetadata, SITE_NAME } from "@/lib/seo";

export const metadata: Metadata = buildPageMetadata({
  title: "Tournois voyage en ligne gratuits",
  description:
    "Crée des brackets et tierlists de pays et destinations. Vote pour tes lieux préférés. TravelKlash — 100 % gratuit.",
  path: "/",
});

export default async function Home() {
  const { t } = await getI18n();
  const topCountries = await getTopCountries(18);
  const marqueeItems = topCountries.length > 0 ? [...topCountries, ...topCountries] : [];
    ...
    <>
      <JsonLd
        data={{
          ...
          url: absoluteUrl(),
          description:
            "Plateforme gratuite voyage : brackets et tierlists autour des pays et destinations.",
        }}
      />
      <div className="space-y-4">
        ...
        >
          <div
            ...
            style={{
              background:
                "radial-gradient(900px 480px at 55% 10%, rgba(56,189,248,0.22) 0%, rgba(14,165,233,0.10) 45%, transparent 78%)",
            }}
          />
          <div className="relative">
            ...
            >
              ...
              <span
                ...
                style={{
                  background: "linear-gradient(90deg, #0ea5e9 5%, #38bdf8 45%, #7dd3fc 85%)",
          ...
          <section className="overflow-hidden rounded-2xl border border-[color:var(--border)] py-4">
            ...
            <div className="flex gap-3 animate-[marquee_40s_linear_infinite] w-max">
              {marqueeItems.map((c, i) => (
                <div
                  key={`${c.id}-${i}`}
                  className="h-20 w-32 shrink-0 overflow-hidden rounded-lg border border-[color:var(--border)] bg-[color:var(--surface-2)]"
                  style={
                    c.coverUrl
                      ? {
