...
import TravelPicker, { type SelectedItem } from "@/components/TravelPicker";
...
export default function CreateTierlistForm() {
  ...
  const onSubmit = (e: React.FormEvent) => {
    ...
    setError(null);
    if (items.length < 2) {
      setError("Il faut au moins 2 destinations.");
      return;
    }
  ...
  return (
    <form onSubmit={onSubmit} className="space-y-6">
      <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
        <div>
          ...
          <Input
            ...
            placeholder="Ex. Pays à visiter en 2026"
          />
        </div>
        <div>
          ...
          <Input
            ...
            placeholder="Ex. Asie du Sud-Est"
          />
        </div>
      </div>
      ...
      <TravelPicker size={64} selected={items} onChange={setItems} freeMode />
