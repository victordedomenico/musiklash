...
export default function TravelPicker({ size, selected, onChange, freeMode = false }: Props) {
  ...
  const { results: cityResults, loading: cityLoading } = useDebouncedSearch(
    "/api/content/search?kind=city",
    query,
    tab === "city",
  );
  ...
  const tabs: { key: Tab; label: string; icon: typeof Globe2 }[] = [
    ...
    { key: "city", label: "Villes", icon: Building2 },
  ];
    ...
    <div className="space-y-4">
      ...
      {tab === "city" && !query.trim() && (
        <p className="text-xs text-[color:var(--muted)]">
          Recherche de villes via OpenTripMap (optionnel — définir{" "}
          <code className="text-[0.7rem]">OPENTRIPMAP_API_KEY</code> côté serveur).
        </p>
      )}
      ...
      <div className="min-h-[200px] space-y-2">
        ...
        {(countryLoading || regionLoading || cityLoading) && (
