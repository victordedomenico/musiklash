...
export default function RetroPicker({ size, selected, onChange, freeMode = false }: Props) {
  const [tab, setTab] = useState<Tab>("game");
  const [query, setQuery] = useState("");
  const [openedCollection, setOpenedCollection] = useState<ContentCollection | null>(null);
  const [openedEra, setOpenedEra] = useState<ContentEntity | null>(null);
  const [collectionGames, setCollectionGames] = useState<ContentItem[]>([]);
  const [eraGames, setEraGames] = useState<ContentItem[]>([]);
  ...
  const { results: eraResults, loading: eraLoading } = useDebouncedSearch(
  ...
  const openEra = async (entity: ContentEntity) => {
    setOpenedEra(entity);
    ...
    try {
      ...
      const json = await res.json();
      setEraGames(json.items ?? json.results ?? []);
    } catch (e) {
      console.error(e);
    } finally {
      setDrillLoading(false);
  ...
  const tabs: { key: Tab; label: string; icon: typeof Gamepad2 }[] = [
    ...
    { key: "collection", label: "Consoles", icon: Layers },
    { key: "era", label: "Ères", icon: Clock },
  ];

  const eraEntities = eraResults as unknown as ContentEntity[];
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="min-h-[200px] space-y-2">
          ...
          (collectionResults as unknown as ContentCollection[]).map((col) => (
            ...
            >
              ...
              <div>
                ...
                <p className="text-xs text-[color:var(--muted)]">Jeux de la console</p>
              </div>
            </button>
          ))}
        ...
        {tab === "era" &&
          ...
          eraEntities.map((entity) => (
            <button
              ...
              type="button"
              className="w-full flex items-center gap-3 rounded-xl border border-[color:var(--border)] p-3 text-left hover:bg-[color:var(--surface-hover)]"
              onClick={() => void openEra(entity)}
            >
              <div className="w-12 h-12 rounded-full bg-[color:var(--surface-2)] flex items-center justify-center">
                <Clock size={20} className="text-[color:var(--muted)]" />
              </div>
              <div>
                <p className="font-semibold">{entity.name}</p>
                <p className="text-xs text-[color:var(--muted)]">Jeux de l&apos;ère</p>
              </div>
            </button>
          ))}
        ...
        {tab === "era" && openedEra && (
          <>
            <p className="text-sm font-medium">{openedEra.name}</p>
            {drillLoading && <p className="text-sm text-[color:var(--muted)]">Chargement…</p>}
            {eraGames.map((item) => (
              <ResultRow
                key={item.id}
        ...
        {(gameLoading || collectionLoading || eraLoading) && (
