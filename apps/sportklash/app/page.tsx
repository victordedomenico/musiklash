...
import { getTopEvents } from "@/lib/top-events";
import { absoluteUrl, buildPageMetadata, SITE_NAME } from "@/lib/seo";

export const metadata: Metadata = buildPageMetadata({
  title: "Tournois sportifs en ligne gratuits",
  description:
    "Crée des brackets et tierlists sportifs. Vote pour tes équipes, joueurs et matchs préférés. SportKlash — 100 % gratuit.",
  path: "/",
});

export default async function Home() {
  const { t } = await getI18n();
  const topEvents = await getTopEvents(18);
  const marqueeItems = topEvents.length > 0 ? [...topEvents, ...topEvents] : [];
    ...
    <>
      <JsonLd
        data={{
          ...
          url: absoluteUrl(),
          description:
            "Plateforme gratuite de sport : brackets et tierlists autour des matchs et équipes.",
        }}
      />
      <div className="space-y-4">
        ...
        >
          <div
            ...
            style={{
              background:
                "radial-gradient(900px 480px at 55% 10%, rgba(34,197,94,0.22) 0%, rgba(22,163,74,0.10) 45%, transparent 78%)",
            }}
          />
          <div className="relative">
            ...
            >
              ...
              <span
                ...
                style={{
                  background: "linear-gradient(90deg, #4ade80 5%, #22c55e 45%, #86efac 85%)",
            ...
            <div className="flex gap-3 animate-[marquee_40s_linear_infinite] w-max">
              {marqueeItems.map((m, i) => (
                ...
                >
                  ...
                  {m.subtitle && (
                    <p className="mt-1 text-[0.55rem] text-[color:var(--muted)] line-clamp-2">
                      {m.subtitle}
                    </p>
                  )}
                </div>
