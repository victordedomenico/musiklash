...
async function seed() {
  ...
  if (adminProfile) bracketsToInsert.push({ ownerId: adminProfile.id, title: 'Top 8 Shōnen All-Time', theme: 'Shōnen', size: 8, visibility: 'public' });
  if (modProfile) bracketsToInsert.push({ ownerId: modProfile.id, title: 'Best Naruto Openings', theme: 'Naruto', size: 16, visibility: 'public' });
  if (userProfile) bracketsToInsert.push({ ownerId: userProfile.id, title: 'Mes Personnages One Piece', theme: 'One Piece', size: 4, visibility: 'private' });
