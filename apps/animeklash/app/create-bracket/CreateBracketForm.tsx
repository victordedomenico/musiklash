...
export default function CreateBracketForm() {
  ...
  const onSubmit: React.FormEventHandler<HTMLFormElement> = (e) => {
    ...
    if (selected.length < 3) {
      setError("Sélectionne au moins 3 éléments.");
      return;
    }
  ...
  return (
    <form onSubmit={onSubmit} className="space-y-6">
      <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
        <div>
          ...
          <Input
            ...
            placeholder="Ex. Top 8 Shōnen All-Time"
          />
        </div>
        <div>
      ...
      <AnimePicker size={size} selected={selected} onChange={setSelected} tabs={["anime", "character"]} />
      ...
      <div className="flex items-center justify-between border-t border-[color:var(--border)] pt-4">
        <p className="text-sm text-[color:var(--muted)]">
          {selected.length} élément{selected.length !== 1 ? "s" : ""} · min. 3
          {effectiveSize ? (
