...
import { getCharacterById } from "@/lib/battle-feat-server";
...
export async function generateMetadata({
  ...
  return { title: s ? `Score ${s.score} pts — BattleClash` : "BattleClash Résultats" };
}
...
export default async function BattleFeatResultsPage({
  ...
  const startingCharInfo = await getCharacterById(session.startingArtistId).catch(() => null);
  const startingCharacter = {
    name: startingCharInfo?.name ?? session.startingArtistId,
    pictureUrl: startingCharInfo?.pictureUrl ?? null,
  };
      ...
      >
        ...
        <div className="space-y-2">
          <div className="flex items-center gap-3 rounded-xl border border-[color:var(--accent)]/30 bg-[color:var(--accent)]/5 px-4 py-2.5">
            ...
            <div>
              <p className="font-semibold">{startingCharacter.name}</p>
