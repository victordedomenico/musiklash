...
export async function generateMetadata({
  ...
  return { title: room ? "Room BattleClash" : "BattleClash" };
}
...
export default async function RoomPage({
  ...
  if (!playerId) {
    redirect(
      `/api/guest/ensure?redirect=${encodeURIComponent(`/battle-feat/room/${roomId}`)}`,
    );
  }
  ...
  return (
    <div className="mx-auto w-full max-w-[1040px] py-6">
      <div
        className="mb-6 rounded-[28px] border px-6 py-5"
        style={{ borderColor: "#2a3242", background: "#10141d" }}
      >
        <h1 className="text-5xl font-black tracking-[-0.03em]">BattleClash Multi</h1>
        <p className="mt-1 text-lg" style={{ color: "#8f93a0" }}>
          Défi en temps réel · Room {roomData.id.slice(0, 8)}
        </p>
      </div>
