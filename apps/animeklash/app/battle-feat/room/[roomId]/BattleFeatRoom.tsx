...
export default function BattleFeatRoom({
  ...
  const disconnectedParticipants = useMemo(() => {
    ...
    return room.participants.filter((p) => {
      if (p.eliminated || p.playerId === userId) return false;
  ...
  const handleDownload = async () => {
    ...
    try {
      await downloadNodeAsPng(exportRef.current, {
        filename: `battleclash-room-${room.id}.png`,
        backgroundColor: "var(--surface)",
      });
    } catch {
      alert("Impossible de générer le PNG pour le moment.");
    } finally {
  ...
  const handleStartGame = async (character: BattleFeatEntity) => {
    setSubmitting(true);
    setError("");
    const result = await startGame(room.id, character.id, character.name, character.pictureUrl);
    if (!result.ok) {
      setError(result.error ?? "Erreur room");
    } else {
      setRoom(result.room);
  ...
  const handleMove = async (character: BattleFeatEntity) => {
    ...
    if (!result.ok) {
      ...
      if (!result.valid) setError(result.error ?? "Coup invalide");
      setRoom(result.room);
      setNow(Date.now());
      await broadcastSync({ room: result.room, event: result.event });
    }
    ...
    return (
      <div className="space-y-6">
        <div className="card p-8 text-center">
          ...
          <h2 className="text-xl font-bold">Room BattleClash</h2>
          ...
          {readyToStart ? (
            <div className="mt-6 space-y-3">
              <p className="text-sm text-[color:var(--muted)]">
                Choisis le personnage de départ pour lancer la partie :
              </p>
              <ArtistSearchInput
                onSelect={handleStartGame}
                disabled={submitting}
                placeholder="Personnage de départ…"
                autoFocus
              />
            </div>
          ) : null}
          ...
          {disconnectedParticipants.length > 0 ? (
            <p className="mt-4 flex items-center justify-center gap-2 text-sm text-amber-300">
              ...
              {disconnectedParticipants.length} joueur{disconnectedParticipants.length > 1 ? "s" : ""} déconnecté{disconnectedParticipants.length > 1 ? "s" : ""}.
            </p>
          ) : null}
        </div>
      ...
      <div className="space-y-6">
        ...
        <div ref={exportRef} className="space-y-6">
          <div className="card p-8 text-center">
            <Trophy
              size={48}
              className={`mx-auto mb-4 ${isDraw ? "text-sky-400" : iWon ? "text-yellow-400" : "text-[color:var(--muted)]"}`}
            />
              ...
              {ranked.map((p, idx) => (
                <div
                  ...
                  style={{
                    ...
                    border: idx === 0 && !isDraw ? "1px solid var(--accent)" : "1px solid var(--border)",
          ...
          <div className="card p-6">
            ...
            <div className="space-y-2">
              {room.startingArtistName ? (
                <CharacterBadge name={room.startingArtistName} pictureUrl={room.startingArtistPic} label="Départ" />
              ) : null}
        ...
        <div className="no-export flex flex-wrap gap-3">
          <button type="button" onClick={handleDownload} disabled={isDownloading} className="btn-ghost flex-1 justify-center disabled:opacity-50">
            <Download size={16} />
            {isDownloading ? "Génération…" : "Enregistrer en PNG"}
          </button>
          <button type="button" onClick={handleSaveAndShare} disabled={sharing} className="btn-primary flex-1 justify-center disabled:opacity-50">
            <Share2 size={16} />
            {sharing ? "…" : "Partager le lien"}
          </button>
          {isParticipant ? (
            <button
              onClick={async () => {
    ...
    <div className="space-y-4">
        ...
        <div className="flex flex-col gap-2 rounded-xl border border-[color:var(--accent)] bg-[color:var(--accent-dim)] px-4 py-2.5 text-sm sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="text-xs text-[color:var(--muted)]">
              Tu observes la partie. Tu pourras rejoindre lors de la prochaine revanche.
            </p>
          </div>
          <button
            type="button"
          ...
          >
            {waitingRematch ? (
              <><Check size={14} className="text-green-400" /> Prêt·e pour la prochaine manche</>
            ) : (
              <><Swords size={14} /> Prêt·e pour la prochaine manche ?</>
            )}
          </button>
        </div>
      ) : null}
      ...
      <div className="card px-4 py-2 text-sm">
        <div className="mb-1 flex items-center justify-between">
          <span className="text-xs font-semibold uppercase tracking-widest text-[color:var(--muted)]">Scores</span>
      ...
      <div className="card space-y-3 p-6">
        <h3 className="text-xs font-bold uppercase tracking-wide text-[color:var(--muted)]">Chaîne</h3>
        ...
        {isMyTurn ? (
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              ...
              <span className={`text-xl font-black tabular-nums ${timeLeft <= 7 ? "animate-pulse text-red-400" : ""}`}>
                {timeLeft}s
              </span>
            </div>
            ...
            {room.currentArtistName ? (
              <div className="space-y-2">
                ...
                <CharacterBadge name={room.currentArtistName} pictureUrl={room.currentArtistPic} label="Courant" />
              </div>
            ) : null}
