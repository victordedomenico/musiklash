...
export async function createBattleFeatChallenge(input: CreateBattleFeatChallengeInput) {
  ...
  if (![1, 2, 3].includes(input.difficulty)) return { error: "Difficulté invalide." };
  if (input.visibility !== "private" && input.visibility !== "public") {
    return { error: "Visibilité invalide." };
  }
  if (!input.startingArtistId || !input.startingArtistName) {
    return { error: "Le personnage de départ est requis." };
  }
  ...
  try {
    ...
    const msg = err instanceof Error ? err.message : "Erreur création BattleClash.";
    return { error: msg };
  }
}
