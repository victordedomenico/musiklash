...
import MangaPicker, { type SelectedItem } from "@/components/MangaPicker";
...
export default function CreateBracketForm() {
  ...
  const onSubmit: React.FormEventHandler<HTMLFormElement> = (e) => {
    ...
    if (selected.length < 3) {
      setError("Sélectionne au moins 3 mangas.");
      return;
    }
  ...
  return (
    <form onSubmit={onSubmit} className="space-y-6">
      <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
        <div>
          ...
          <Input
            ...
            placeholder="Ex. Meilleurs shōnen"
          />
        </div>
        <div>
      ...
      <MangaPicker size={size} selected={selected} onChange={setSelected} />
