...
import { getTopMeals } from "@/lib/top-meals";
import { absoluteUrl, buildPageMetadata, SITE_NAME } from "@/lib/seo";

export const metadata: Metadata = buildPageMetadata({
  title: "Tournois plats en ligne gratuits",
  description:
    "Crée des brackets et tierlists culinaires. Vote pour tes plats, cuisines et catégories préférés. FoodKlash — 100 % gratuit.",
  path: "/",
});

export default async function Home() {
  const { t } = await getI18n();
  const topMeals = await getTopMeals(18);
  const marqueeItems = topMeals.length > 0 ? [...topMeals, ...topMeals] : [];
    ...
    <>
      <JsonLd
        data={{
          ...
          url: absoluteUrl(),
          description:
            "Plateforme gratuite de jeux autour de la cuisine : brackets et tierlists de plats.",
        }}
      />
      <div className="space-y-4">
        ...
        >
          <div
            ...
            style={{
              background:
                "radial-gradient(900px 480px at 55% 10%, rgba(249,115,22,0.22) 0%, rgba(251,146,60,0.10) 45%, transparent 78%)",
            }}
          />
          ...
          <section className="overflow-hidden rounded-2xl border border-[color:var(--border)] py-4">
            ...
            <div className="flex gap-3 animate-[marquee_40s_linear_infinite] w-max">
              {marqueeItems.map((m, i) => (
                <div
                  key={`${m.id}-${i}`}
                  className="h-28 w-28 shrink-0 overflow-hidden rounded-lg border border-[color:var(--border)] bg-[color:var(--surface-2)]"
                  style={
                    m.coverUrl
