...
import { Search, Plus, X, ChevronLeft, Check, UtensilsCrossed, Tag, Globe, Apple, Carrot, ShoppingBasket, Fish, Beef, Store, GlassWater, Martini } from "lucide-react";
...
type Tab = "meal" | "category" | "cuisine" | "ingredient" | "fruit" | "vegetable" | "fish" | "meat" | "fastfood" | "drink" | "drinktype" | "cocktail";
...
const DRINK_CATEGORIES = [
  ...
  { slug: "en:energy-drinks",    label: "Énergisantes",     emoji: "⚡" },
...
export default function FoodPicker({ size, selected, onChange, freeMode = false }: Props) {
  ...
  const [drinkTypeItems, setDrinkTypeItems] = useState<ContentItem[]>([]);
  const [drinkTypeLoading, setDrinkTypeLoading] = useState(false);
  ...
  const tabs: { key: Tab; label: string; icon: typeof UtensilsCrossed }[] = [
    ...
    { key: "drink", label: "Boissons", icon: GlassWater },
    { key: "drinktype", label: "Boissons par type", icon: GlassWater },
    { key: "cocktail", label: "Cocktails", icon: Martini },
  ];
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="min-h-[200px] space-y-2">
            ...
            {DRINK_CATEGORIES.map((cat) => (
              <button key={cat.slug} type="button"
                ...
                onClick={() => {
                  ...
                  setDrinkTypeLoading(true);
                  setDrinkTypeItems([]);
                  fetch(`/api/drinks/category/${encodeURIComponent(cat.slug)}`)
                    .then((r) => r.json())
                    .then((json) => setDrinkTypeItems(json.results ?? []))
                    .catch(console.error)
                    .finally(() => setDrinkTypeLoading(false));
                }}>
                <span className="text-xl shrink-0">{cat.emoji}</span>
                <span className="font-medium text-sm truncate flex-1">{cat.label}</span>
        ...
        {tab === "drinktype" && openedDrinkType && (
          <>
            <button type="button" className="btn-ghost text-sm inline-flex items-center gap-1"
              onClick={() => { setOpenedDrinkType(null); setDrinkTypeItems([]); }}>
              <ChevronLeft size={16} /> Retour
            </button>
            <p className="text-sm font-medium text-[color:var(--muted)]">{openedDrinkType.emoji} {openedDrinkType.label}</p>
            {drinkTypeLoading && <p className="text-sm text-[color:var(--muted)]">Chargement…</p>}
            {!drinkTypeLoading && drinkTypeItems.length === 0 && (
              <p className="text-sm text-[color:var(--muted)]">Aucune boisson trouvée.</p>
            )}
            {drinkTypeItems.map((item) => (
              <ResultRow key={item.id} title={item.title} subtitle={item.subtitle}
                coverUrl={item.coverUrl} selected={isSelected(item.id)} onAdd={() => addItem(item)} />
            ))}
          </>
        ...
        {(mealLoading || categoryLoading || cuisineLoading || ingredientLoading || fruitLoading || vegetableLoading || fishLoading || meatLoading || drinkLoading || cocktailLoading) && (
