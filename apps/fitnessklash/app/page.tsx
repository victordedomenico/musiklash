...
import { getTopExercises } from "@/lib/top-exercises";
import { absoluteUrl, buildPageMetadata, SITE_NAME } from "@/lib/seo";

export const metadata: Metadata = buildPageMetadata({
  title: "Tournois d'exercices et fitness gratuits",
  description:
    "Crée des brackets et tierlists d'exercices, muscles et programmes. Vote pour tes mouvements préférés. FitnessKlash — 100 % gratuit.",
  path: "/",
});

export default async function Home() {
  const { t } = await getI18n();
  const topExercises = await getTopExercises(18);
  const marqueeItems = topExercises.length > 0 ? [...topExercises, ...topExercises] : [];
    ...
    <>
      <JsonLd
        data={{
          ...
          url: absoluteUrl(),
          description:
            "Plateforme gratuite pour brackets et tierlists d'exercices, muscles et programmes fitness.",
        }}
      />
