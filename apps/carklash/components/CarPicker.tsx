...
function itemToSelected(item: ContentItem): SelectedItem {
  return {
    ...
    preview_url: null,
    source: item.source ?? "carquery",
    metadata: { itemKind: item.metadata?.itemKind ?? "model", ...item.metadata },
  };
}

export default function CarPicker({ size, selected, onChange, freeMode = false }: Props) {
  const [tab, setTab] = useState<Tab>("model");
  const [query, setQuery] = useState("");
  const [openedBody, setOpenedBody] = useState<ContentCollection | null>(null);
  const [openedMake, setOpenedMake] = useState<ContentEntity | null>(null);
  const [bodyModels, setBodyModels] = useState<ContentItem[]>([]);
  const [makeModels, setMakeModels] = useState<ContentItem[]>([]);
  ...
  const { results: bodyResults, loading: bodyLoading } = useDebouncedSearch(
    "/api/content/search?kind=collections",
    query,
    tab === "body" && !openedBody,
  );

  const { results: makeResults, loading: makeLoading } = useDebouncedSearch(
  ...
  const openBody = async (col: ContentCollection) => {
    setOpenedBody(col);
    setOpenedMake(null);
    ...
    try {
      ...
      const json = await res.json();
      setBodyModels(json.items ?? json.results ?? []);
    } catch (e) {
      console.error(e);
    } finally {
  ...
  const openMake = async (entity: ContentEntity) => {
    setOpenedMake(entity);
    setOpenedBody(null);
    setDrillLoading(true);
    setMakeModels([]);
    try {
      ...
      const json = await res.json();
      setMakeModels(json.items ?? json.results ?? []);
    } catch (e) {
      console.error(e);
    } finally {
      setDrillLoading(false);
  ...
  const tabs: { key: Tab; label: string; icon: typeof Car }[] = [
    { key: "model", label: "Modèles", icon: Car },
    { key: "body", label: "Carrosseries", icon: Layers },
    { key: "make", label: "Marques", icon: Building2 },
  ];
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="min-h-[200px] space-y-2">
        ...
        {tab === "body" && !openedBody &&
          (bodyResults as unknown as ContentCollection[]).map((col) => (
            <button
              key={col.id}
              type="button"
              ...
              onClick={() => void openBody(col)}
            >
              <div className="w-12 h-12 rounded-md bg-[color:var(--surface-2)] flex items-center justify-center text-lg">
                🚗
              </div>
              <div>
                <p className="font-semibold">{col.title}</p>
                <p className="text-xs text-[color:var(--muted)]">Voir les modèles</p>
              </div>
            </button>
          ))}
        ...
        {tab === "body" && openedBody && (
          <>
            <p className="text-sm font-medium text-[color:var(--muted)]">{openedBody.title}</p>
        ...
        {tab === "make" && !openedMake &&
          (makeResults as unknown as ContentEntity[]).map((entity) => (
            <button
              key={entity.id}
              type="button"
              className="w-full flex items-center gap-3 rounded-xl border border-[color:var(--border)] p-3 text-left hover:bg-[color:var(--surface-hover)]"
              onClick={() => void openMake(entity)}
            >
              <div className="w-12 h-12 rounded-full bg-[color:var(--surface-2)] flex items-center justify-center text-sm font-bold">
        ...
        {tab === "make" && openedMake && (
          <>
            <p className="text-sm font-medium">{openedMake.name}</p>
            {drillLoading && <p className="text-sm text-[color:var(--muted)]">Chargement…</p>}
            {makeModels.map((item) => (
              <ResultRow
                key={item.id}
        ...
        {(modelLoading || bodyLoading || makeLoading) && (
...
function ResultRow({
  ...
  return (
    <div className="flex items-center gap-3 rounded-xl border border-[color:var(--border)] p-3">
      <div className="w-10 h-10 rounded bg-[color:var(--surface-2)] shrink-0 flex items-center justify-center text-base">
        🚗
      </div>
