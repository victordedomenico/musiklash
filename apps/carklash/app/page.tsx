...
import { getTopCars } from "@/lib/top-cars";
import { absoluteUrl, buildPageMetadata, SITE_NAME } from "@/lib/seo";

export const metadata: Metadata = buildPageMetadata({
  title: "Tournois automobiles en ligne gratuits",
  description:
    "Crée des brackets et tierlists de modèles et marques. Vote pour tes voitures préférées. CarKlash — 100 % gratuit.",
  path: "/",
});

export default async function Home() {
  const { t } = await getI18n();
  const topCars = await getTopCars(18);
  const marqueeItems = topCars.length > 0 ? [...topCars, ...topCars] : [];
    ...
    <>
      <JsonLd
        data={{
          ...
          url: absoluteUrl(),
          description:
            "Plateforme gratuite automobile : brackets et tierlists autour des modèles et marques.",
        }}
      />
      <div className="space-y-4">
        ...
        >
          <div
            ...
            style={{
              background:
                "radial-gradient(900px 480px at 55% 10%, rgba(249,115,22,0.22) 0%, rgba(239,68,68,0.10) 45%, transparent 78%)",
            }}
          />
          ...
          <section className="overflow-hidden rounded-2xl border border-[color:var(--border)] py-4">
            ...
            <div className="flex gap-3 animate-[marquee_40s_linear_infinite] w-max">
              {marqueeItems.map((c, i) => (
                <div
                  key={`${c.id}-${i}`}
                  className="flex h-20 w-36 shrink-0 flex-col items-center justify-center gap-1 overflow-hidden rounded-lg border border-[color:var(--border)] bg-[color:var(--surface-2)] px-2 text-center"
                  title={c.subtitle ? `${c.title} · ${c.subtitle}` : c.title}
                >
                  <span className="text-xl" aria-hidden>
                    🚗
                  </span>
                  <span className="line-clamp-1 text-xs font-semibold">{c.title}</span>
                  {c.subtitle && (
                    <span className="line-clamp-1 text-[0.65rem] text-[color:var(--muted)]">
                      {c.subtitle}
                    </span>
                  )}
                </div>
