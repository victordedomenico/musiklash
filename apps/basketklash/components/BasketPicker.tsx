...
export default function BasketPicker({ size, selected, onChange, freeMode = false }: Props) {
  ...
  const [seasonGames, setSeasonGames] = useState<ContentItem[]>([]);
  ...
  const openSeason = async (col: ContentCollection) => {
    ...
    try {
      ...
      const json = await res.json();
      setSeasonGames(json.items ?? json.results ?? []);
    } catch (e) {
      console.error(e);
    } finally {
  ...
  const tabs: { key: Tab; label: string; icon: typeof Swords }[] = [
    ...
    { key: "season", label: "Saisons", icon: Trophy },
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="min-h-[200px] space-y-2">
        ...
        {(matchLoading || seasonLoading || teamLoading) && (
