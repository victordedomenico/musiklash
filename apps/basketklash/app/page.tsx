...
import { getTopGames } from "@/lib/top-games";
import { absoluteUrl, buildPageMetadata, SITE_NAME } from "@/lib/seo";

export const metadata: Metadata = buildPageMetadata({
  title: "Tournois NBA en ligne gratuits",
  description:
    "Crée des brackets et tierlists NBA. Vote pour tes équipes, joueurs et matchs préférés. BasketKlash — 100 % gratuit.",
  path: "/",
});

export default async function Home() {
  const { t } = await getI18n();
  const topGames = await getTopGames(18);
  const marqueeItems = topGames.length > 0 ? [...topGames, ...topGames] : [];
    ...
    <>
      <JsonLd
        data={{
          ...
          url: absoluteUrl(),
          description:
            "Plateforme gratuite de basketball : brackets et tierlists autour des matchs et équipes NBA.",
        }}
      />
      <div className="space-y-4">
        ...
        >
          <div
            ...
            style={{
              background:
                "radial-gradient(900px 480px at 55% 10%, rgba(234,88,12,0.22) 0%, rgba(249,115,22,0.10) 45%, transparent 78%)",
            }}
          />
          <div className="relative">
            ...
            >
              ...
              <span
                ...
                style={{
                  background: "linear-gradient(90deg, #ea580c 5%, #f97316 45%, #fdba74 85%)",
          ...
          <section className="overflow-hidden rounded-2xl border border-[color:var(--border)] py-4">
            ...
            <div className="flex gap-3 animate-[marquee_40s_linear_infinite] w-max">
              {marqueeItems.map((m, i) => (
                <div
                  key={`${m.id}-${i}`}
                  className="flex h-28 w-28 shrink-0 flex-col justify-end overflow-hidden rounded-lg border border-[color:var(--border)] bg-[color:var(--surface-2)] p-2 text-left"
                  title={m.title}
                >
                  <p className="text-[0.65rem] font-bold leading-tight line-clamp-3">{m.title}</p>
                  {m.subtitle && (
                    <p className="mt-1 text-[0.55rem] text-[color:var(--muted)] line-clamp-2">{m.subtitle}</p>
                  )}
                </div>
              ))}
            </div>
