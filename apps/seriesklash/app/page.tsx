...
import { getTopSeries } from "@/lib/top-series";
import { absoluteUrl, buildPageMetadata, SITE_NAME } from "@/lib/seo";

export const metadata: Metadata = buildPageMetadata({
  title: "Tournois séries TV en ligne gratuits",
  description:
    "Crée des brackets et tierlists autour de tes séries préférées. Vote pour tes shows, saisons et épisodes. SeriesKlash — 100 % gratuit.",
  path: "/",
});

export default async function Home() {
  const { t } = await getI18n();
  const topSeries = await getTopSeries(18);
  const marqueeItems = topSeries.length > 0 ? [...topSeries, ...topSeries] : [];
    ...
    <>
      <JsonLd
        data={{
          ...
          url: absoluteUrl(),
          description:
            "Plateforme gratuite de jeux autour des séries TV : brackets et tierlists.",
        }}
      />
      <div className="space-y-4">
        ...
        >
          <div
            ...
            style={{
              background:
                "radial-gradient(900px 480px at 55% 10%, rgba(99,102,241,0.22) 0%, rgba(129,140,248,0.10) 45%, transparent 78%)",
            }}
          />
          <div className="relative">
            ...
            >
              ...
              <span
                ...
                style={{
                  background: "linear-gradient(90deg, #6366f1 5%, #818cf8 45%, #c7d2fe 85%)",
            ...
            <div className="flex gap-3 animate-[marquee_40s_linear_infinite] w-max">
              {marqueeItems.map((s, i) => (
                <div
                  ...
                  style={
                    s.coverUrl
                      ? { backgroundImage: `url(${s.coverUrl})`, backgroundSize: "cover", backgroundPosition: "center" }
                      : undefined
                  }
                  title={s.title}
                />
