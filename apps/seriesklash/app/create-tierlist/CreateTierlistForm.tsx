...
import SeriesPicker, { type SelectedItem } from "@/components/SeriesPicker";
...
export default function CreateTierlistForm() {
  ...
  const onSubmit = (e: React.FormEvent) => {
    ...
    setError(null);
    if (items.length < 2) {
      setError("Il faut au moins 2 séries ou épisodes.");
      return;
    }
  ...
  return (
    <form onSubmit={onSubmit} className="space-y-6">
      <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
        <div>
          ...
          <Input
            ...
            placeholder="Ex. Séries sci-fi des années 2010"
          />
        </div>
        <div>
      ...
      <SeriesPicker size={64} selected={items} onChange={setItems} freeMode />
