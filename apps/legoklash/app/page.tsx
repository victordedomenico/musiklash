...
import { getTopSets } from "@/lib/top-sets";
import { absoluteUrl, buildPageMetadata, SITE_NAME } from "@/lib/seo";

export const metadata: Metadata = buildPageMetadata({
  title: "Tournois LEGO en ligne gratuits",
  description:
    "Crée des brackets et tierlists LEGO. Vote pour tes sets, licences et minifigs préférés. LegoKlash — 100 % gratuit.",
  path: "/",
});

export default async function Home() {
  const { t } = await getI18n();
  const topSets = await getTopSets(18);
  const marqueeItems = topSets.length > 0 ? [...topSets, ...topSets] : [];
    ...
    <>
      <JsonLd
        data={{
          ...
          url: absoluteUrl(),
          description:
            "Plateforme gratuite LEGO : brackets et tierlists autour des sets.",
        }}
      />
      <div className="space-y-4">
        ...
        >
          <div
            ...
            style={{
              background:
                "radial-gradient(900px 480px at 55% 10%, rgba(234,179,8,0.22) 0%, rgba(220,38,38,0.10) 45%, transparent 78%)",
            }}
          />
