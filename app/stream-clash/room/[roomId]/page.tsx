...
export default async function StreamClashRoomPage({
  ...
  if (!playerId) {
    redirect(`/api/guest/ensure?redirect=${encodeURIComponent(`/stream-clash/room/${roomId}`)}`);
  }
  ...
  return (
    <div className="page-shell max-w-3xl py-10">
      ...
      <StreamClashRoomClient initialRoom={room} userId={playerId} username={username} />
    </div>
  );
}
