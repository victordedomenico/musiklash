...
type ProfileRecord = { id: string; username: string };

type GuestAuthDeps = {
  prisma: any;
  createClient: () => Promise<any>;
  fallbackGuestEmailDomain: string;
};
...
const ANIMALS = ["Fox", "Panda", "Wolf", "Tiger", "Falcon", "Otter", "Lynx", "Raven", "Koala", "Panther"];
...
export function createGuestIdentityHelpers(deps: GuestAuthDeps) {
  const { prisma, createClient, fallbackGuestEmailDomain } = deps;

  function isFallbackGuestEmail(email: string | null | undefined): boolean {
    if (!email) return false;
    return email.endsWith(`@${fallbackGuestEmailDomain}`);
  }

  function buildFallbackGuestCredentials() {
    const token = crypto.randomUUID().replace(/-/g, "");
    return {
      email: `guest_${token}@${fallbackGuestEmailDomain}`,
  ...
  async function getProfileWithRetry(userId: string, maxAttempts = 5): Promise<ProfileRecord | null> {
  ...
  async function ensureAnonymousUser() {
    ...
    if (signUpData.session) return signUpData.user;
  ...
  async function getGuestIdentityFromCookies() {
    const supabase = await createClient();
    const {
      data: { user },
    } = await supabase.auth.getUser();
    ...
    if (user && (user.is_anonymous || isFallbackGuestEmail(user.email))) {
      ...
      return { id: user.id, username: profile?.username ?? "Anonyme" };
    }
  ...
  async function resolvePlayerIdentity() {
    ...
    if (cookieGuest) {
      return { playerId: cookieGuest.id, username: cookieGuest.username, isGuest: true };
    }
    ...
    return { playerId: profile.id, username: finalUsername, isGuest };
  }

  async function setGuestUsername(preferredUsername: string) {
    const normalized = normalizePreferredUsername(preferredUsername);
    if (!normalized) return { error: "Pseudo invalide (3 caractères minimum)." };
    ...
    const profile = await getProfileWithRetry(user.id);
    if (!profile) return { error: "Profil invité introuvable." };
  ...
  async function migrateGuestDataToUser(userId: string) {
    ...
    if (!userProfile) {
      ...
      await prisma.profile.create({ data: { id: userId, username: fallbackUsername } });
    }
