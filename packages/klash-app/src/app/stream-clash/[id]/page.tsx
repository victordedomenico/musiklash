...
import { Music, Headphones, Play, Users, ArrowLeft } from "lucide-react";
...
export default async function StreamClashPage({
  ...
  return (
    <div className="page-shell max-w-2xl py-10">
      ...
      <SectionHeader title={sc.title} subtitle={`${sc.tracks.length} ${process.env.NEXT_PUBLIC_KLASH_ITEMS_NOUN ?? "éléments"} · Stream Clash`} />
        ...
        {sc.tracks.map((t) => (
          ...
          >
            {t.coverUrl ? (
              ...
              >
                <MediaIcon size={16} className="text-[color:var(--muted)]" />
              </div>
            )}
