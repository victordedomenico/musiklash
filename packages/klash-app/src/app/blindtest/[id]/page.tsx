...
import { createBlindtestRoom } from "@klash/klash-app/app/blindtest/room/new/actions";
import { User, Users, ArrowRight, Music, Headphones } from "lucide-react";
const MediaIcon = process.env.NEXT_PUBLIC_KLASH_MEDIA_ICON === "headphones" ? Headphones : Music;
...
export async function generateMetadata({
  ...
  return buildPageMetadata({
    title: bt.title,
    description: `Testez vos oreilles avec le blindtest « ${bt.title} » sur ${process.env.NEXT_PUBLIC_KLASH_NAME ?? "Klash"}. Devinez les extraits à l'aveugle.`,
    path: `/blindtest/${id}`,
  ...
  return (
    <div className="page-shell max-w-4xl py-10">
      ...
      <div className="mb-10 text-center">
        ...
        >
          <MediaIcon size={28} style={{ color: "var(--accent)" }} />
        </div>
        <h1 className="text-4xl font-black">{blindtest.title}</h1>
        <p className="mt-2 text-[color:var(--muted)]">
          {trackCount} {trackCount > 1 ? (process.env.NEXT_PUBLIC_KLASH_BLINDTEST_ITEMS_NOUN ?? "morceaux") : (process.env.NEXT_PUBLIC_KLASH_BLINDTEST_ITEM_NOUN ?? "morceau")} · 30 secondes par extrait
        </p>
        {multi === "unavailable" ? (
