...
import type { StreamClashParticipant } from "@klash/klash-app/lib/stream-clash-room";

import type { SelectedContentItem } from "@klash/klash-app/lib/content-item";
import { selectedItemToStreamClashTrack } from "@klash/klash-app/lib/content-item";
...
export async function createStreamClash(input: {
  ...
  if (input.tracks.length < 4) return { error: "Il faut au moins 4 éléments." };
  if (input.tracks.length > 50) return { error: "50 éléments maximum." };
      ...
      data: {
        ...
        tracks: {
          create: input.tracks.map((t, i) => ({
            ...selectedItemToStreamClashTrack(t, i, t.rank ?? i),
          })),
        },
      },
    });
