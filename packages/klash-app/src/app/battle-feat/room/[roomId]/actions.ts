...
import { pickJokerMoveForBattleFeat, getBattleFeatGraph } from "@klash/klash-app/lib/battle-feat/graph-helpers";
import { getBattleFeatRoomSnapshot } from "@klash/klash-app/lib/battle-feat/room-snapshot";
import type { BattleFeatParticipant, RoomEvent } from "@klash/klash-app/lib/battle-feat/types";
...
async function buildRoomResponse(roomId: string, event?: RoomEvent) {
  const room = await getBattleFeatRoomSnapshot(prisma, roomId);
...
async function applyMoveToRoom(
  ...
  const usedArtistIds = [...normalizeUsedArtistIds(room.usedArtistIds), entity.id];
  const moves = [
  ...
  await prisma.battleFeatRoom.update({
    where: { id: roomId },
    data: {
      currentArtistId: entity.id,
      currentArtistName: entity.name,
      currentArtistPic: entity.pictureUrl,
...
export async function joinRoom(roomId: string) {
  ...
  if (findBattleFeatParticipant(participants, user.id)) {
    const snap = await getBattleFeatRoomSnapshot(prisma, roomId);
...
export async function startGame(
  ...
  // starting entity, so let a guest move first).
  ...
  await prisma.battleFeatRoom.update({
    ...
    data: {
      status: "playing",
      winnerId: null,
      startingArtistId: startingEntityId,
      startingArtistName: startingEntityName,
      startingArtistPic: startingEntityPic,
      currentArtistId: startingEntityId,
      currentArtistName: startingEntityName,
      currentArtistPic: startingEntityPic,
      currentTurnId: firstTurnId,
      usedArtistIds: [startingEntityId],
      moves: [],
...
export async function submitMove(roomId: string, entity: MoveEntityInput) {
  const user = await requirePlayer();
  ...
  if (!room.currentArtistId || !room.currentArtistName) {
    return { ok: false, valid: false, error: "Pas d'entité courante" } as const;
  }

  const usedArtistIds = normalizeUsedArtistIds(room.usedArtistIds);
  if (usedArtistIds.includes(entity.id)) {
  ...
  const feat = typeof graph.validateWithNames === "function"
    ? await graph.validateWithNames(
        room.currentArtistId,
        room.currentArtistName ?? "",
        entity.id,
        entity.name,
      )
    : await graph.validateLink(room.currentArtistId, entity.id);
  ...
  const moveResult = await applyMoveToRoom(
    ...
    room,
    entity,
    (feat.valid ? (feat.viaItemId ?? null) : null),
    (feat.valid && "previewUrl" in feat ? ((feat as {previewUrl?: string | null}).previewUrl ?? null) : null),
  );
  ...
  return {
    ...
    valid: true,
    trackTitle: moveResult.trackTitle,
    previewUrl: (feat.valid && "previewUrl" in feat ? ((feat as {previewUrl?: string | null}).previewUrl ?? null) : null),
    room: response.room,
    event: response.event,
  } as const;
}
...
export async function useJoker(roomId: string) {
  ...
  if (!room.currentArtistId) return { ok: false, error: "Pas d'entité courante" } as const;

  const participants = normalizeBattleFeatParticipants(room.participants);
  const me = findBattleFeatParticipant(participants, user.id);
  if (!me || me.jokers < 1) return { ok: false, error: "Plus de joker" } as const;

  const jokerArtist = await pickJokerMoveForBattleFeat(prisma,
    room.currentArtistId,
