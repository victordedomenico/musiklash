...
import EntitySearchInput from "@klash/klash-app/components/EntitySearchInput";
import ChallengeOutcomeFx from "@klash/klash-app/components/ChallengeOutcomeFx";
import type { BattleFeatEntity, FeatMove } from "@klash/klash-app/lib/battle-feat/types";
import { saveSoloSession } from "./actions";
import { downloadNodeAsPng } from "@klash/klash-app/lib/download-png";
...
export default function BattleFeatSolo({
  ...
  const initialEntity: BattleFeatEntity | null = challenge
    ? {
        id: challenge.startingEntity.id,
        name: challenge.startingEntity.name,
        nameSlug: "",
        fanCount: 0,
        popularityTier: 3,
        previewUrl: null,
        pictureUrl: challenge.startingEntity.pictureUrl,
      }
    : null;

  const [difficulty, setDifficulty] = useState(challenge?.difficulty ?? 2);
  const [startingEntity, setStartingEntity] = useState<BattleFeatEntity | null>(initialEntity);
  ...
  const [easyOptions, setEasyOptions] = useState<BattleFeatEntity[]>([]);
  ...
  const currentEntityId = lastMove?.artistId ?? startingEntity?.id ?? "";
  const currentEntityName = lastMove?.artistName ?? startingEntity?.name ?? "";
  const usedIds = [
    ...(startingEntity ? [startingEntity.id] : []),
  ...
  const endGame = useCallback(
    ...
    [difficulty, startingEntity, jokersUsed, publishMode, challenge],
  );
  ...
  useEffect(() => {
    if (phase !== "player-turn" || difficulty !== 1 || !currentEntityId) {
      return;
    }
    ...
    void (async () => {
      try {
        const res = await fetch("/api/battle-feat/options", {
          ...
          body: JSON.stringify({ currentEntityId, usedIds }),
        });
        const json = (await res.json()) as { options: BattleFeatEntity[] };
        if (!cancelled) {
  ...
  }, [phase, difficulty, currentEntityId, usedIdsKey]); // eslint-disable-line react-hooks/exhaustive-deps
    ...
    async (
      ...
      try {
        const res = await fetch("/api/battle-feat/ai-move", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ currentEntityId: artistId, difficulty, usedIds: currentUsedIds }),
        });
  ...
  // ── Submit player move ────────────────────────────────────────────────────
  async function submitMove(artist: BattleFeatEntity, options?: { consumeJoker?: boolean }) {
    ...
    try {
      const res = await fetch("/api/battle-feat/validate", {
        ...
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          prevArtistId: currentEntityId,
          prevArtistName: currentEntityName,
      ...
      if (!json.valid) {
        await endGame(
          `${artist.name} n'a pas de featuring connu avec ${currentEntityName}.`,
          "ai",
          moves,
          playerScore,
          finalJokersUsed,
  ...
  async function handleJoker() {
    ...
    try {
      ...
      const jokerArtist: BattleFeatEntity = {
        id: json.artist.id,
        ...
        <div className="card p-6 md:p-8">
          ...
          {startingEntity ? (
            <div className="flex items-center gap-3">
              <EntityChip name={startingEntity.name} pictureUrl={startingEntity.pictureUrl} />
              {!challengeLocked && (
                <button
                  type="button"
        ...
        <div className="flex items-center justify-between">
          <p className="text-sm text-[color:var(--muted)]">
            {isAiThinking ? (
              ...
              <>
                Trouve un feat avec{" "}
                <strong className="text-white">{currentEntityName}</strong>
              </>
            )}
          </p>
          {isPlayerTurn && (
            ...
            {difficulty === 1 ? (
              <div className="space-y-3">
                ...
                <EntitySearchInput
                  onSelect={submitMove}
                  excludeIds={usedIds}
                  placeholder={`Ou cherche un artiste avec ${currentEntityName}…`}
                  autoFocus
                />
              </div>
            ) : (
              <EntitySearchInput
                onSelect={submitMove}
                excludeIds={usedIds}
                placeholder={`Artiste avec un feat avec ${currentEntityName}…`}
                autoFocus
              />
            )}
            {jokers > 0 && (
