...
import {
  ...
  type BracketSize,
  type Vote,
} from "@klash/klash-app/lib/bracket";
...
export default function BracketGame({
  ...
  // Preview URLs may expire. Once the bracket is
  ...
  const togglePreview = (track: BracketTrack) => {
    ...
    // from the DB may be stale (preview URLs can expire
  ...
  if (!a || !b) {
    return (
      <div className="card p-6 text-center text-red-400">
        Impossible de trouver les ${process.env.NEXT_PUBLIC_KLASH_ITEMS_NOUN ?? "éléments"} du duel.
      </div>
    );
  }
