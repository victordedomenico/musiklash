...
import { isCorrect, isSingleArtistBlindtest } from "@klash/klash-app/lib/blindtest-utils";
            ...
            <div className="flex-1 w-full space-y-4">
              {singleArtistMode ? (
                <p className="rounded-xl border px-3 py-2 text-sm text-[color:var(--muted)]" style={{ borderColor: "#2a3242", background: "#131822" }}>
                  Un seul {process.env.NEXT_PUBLIC_KLASH_BLINDTEST_ARTIST_LABEL ?? "artiste"} sur tout le blindtest : devine uniquement le{" "}
                  <strong className="text-[color:var(--foreground)]">titre</strong>. Les{" "}
                  <strong className="text-[color:var(--foreground)]">+{POINTS_ARTIST} pt</strong>{" "}
                  {process.env.NEXT_PUBLIC_KLASH_BLINDTEST_ARTIST_LABEL ?? "artiste"} sont
                  attribués automatiquement.
                </p>
              ) : null}
              <div>
                <label className="text-xs font-semibold uppercase tracking-wide text-[color:var(--muted)]">
                  {process.env.NEXT_PUBLIC_KLASH_BLINDTEST_TITLE_LABEL ?? "Titre"} (+{POINTS_TITLE} pts)
                </label>
                <input
                  ref={titleInputRef}
              ...
              {!singleArtistMode ? (
                <div>
                  <label className="text-xs font-semibold uppercase tracking-wide text-[color:var(--muted)]">
                    {process.env.NEXT_PUBLIC_KLASH_BLINDTEST_ARTIST_LABEL ?? "Artiste"} (+{POINTS_ARTIST} pt)
                  </label>
                  <input
                    className="input mt-1"
              ...
              <div className="flex gap-2 pt-1">
                ...
                <button
                  ...
                  onClick={submit}
                  className="btn-ghost"
                  title={`Passer cet ${process.env.NEXT_PUBLIC_KLASH_BLINDTEST_ITEM_NOUN ?? "élément"}`}
                >
                  <SkipForward size={16} />
                </button>
              </div>
        ...
        {phase === "revealed" && lastAnswer && (
          <div className="flex flex-col md:flex-row gap-8 items-start">
            ...
            <div className="flex-1 w-full space-y-3">
              <AnswerRow
                label={`${process.env.NEXT_PUBLIC_KLASH_BLINDTEST_TITLE_LABEL ?? "Titre"} (+${POINTS_TITLE} pts)`}
                truth={track.title}
              ...
              <AnswerRow
                label={
                  singleArtistMode
                    ? `${process.env.NEXT_PUBLIC_KLASH_BLINDTEST_ARTIST_LABEL ?? "Artiste"} (+${POINTS_ARTIST} pt) · commun à tous les titres`
                    : `${process.env.NEXT_PUBLIC_KLASH_BLINDTEST_ARTIST_LABEL ?? "Artiste"} (+${POINTS_ARTIST} pt)`
                }
                truth={track.artist}
