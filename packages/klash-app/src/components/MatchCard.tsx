...
export default function MatchCard({
  ...
  // Fresh preview URLs fetched on mount (les URLs signées peuvent expirer)
  const [previewA, setPreviewA] = useState(a.preview_url);
  const [previewB, setPreviewB] = useState(b.preview_url);

  useEffect(() => {
    fetchContentItemPreview(a.externalId).then((url) => { if (url) setPreviewA(url); }).catch(() => {});
    fetchContentItemPreview(b.externalId).then((url) => { if (url) setPreviewB(url); }).catch(() => {});
