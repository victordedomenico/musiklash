...
const MODE_BY_OPTION_KEY: Record<(typeof MAIN_OPTIONS)[number]["key"], GameMode> = {
  bracket: "bracket",
  tierlist: "tierlist",
  blindtest: "blindtest",
  streamclash: "stream-clash",
  smashpass: "smash-pass",
  battlefeat: "battle-feat",
};

function enabledMainOptions() {
  const slug = process.env.NEXT_PUBLIC_KLASH_VERTICAL ?? "musiklash";
  const manifest = VERTICAL_MANIFESTS[slug];
  if (!manifest) return [...MAIN_OPTIONS];
  const enabled = new Set(manifest.gameModes);
  return MAIN_OPTIONS.filter((opt) => enabled.has(MODE_BY_OPTION_KEY[opt.key]));
}

function blindtestMenuIcon() {
  return process.env.NEXT_PUBLIC_KLASH_VERTICAL === "animeklash" ? Headphones : Music;
}
    ...
    >
      ...
      {step === "main" && (
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
          {enabledMainOptions().map((opt) => {
            const Icon = opt.key === "blindtest" ? blindtestMenuIcon() : opt.icon;
            const commonStyle = {
              borderColor: "var(--border)",
