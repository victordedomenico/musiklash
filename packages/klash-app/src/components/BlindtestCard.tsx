...
import LibraryVisibilityToggle from "@klash/klash-app/components/LibraryVisibilityToggle";

const mediaIcon = process.env.NEXT_PUBLIC_KLASH_MEDIA_ICON ?? "music";
const MediaIcon = mediaIcon === "headphones" ? Headphones : Music;
const itemNoun = process.env.NEXT_PUBLIC_KLASH_BLINDTEST_ITEM_NOUN ?? "morceau";
const itemsNoun = process.env.NEXT_PUBLIC_KLASH_BLINDTEST_ITEMS_NOUN ?? "morceaux";
  ...
  const inner = (
    <>
      <div className="media-thumb flex items-center justify-center">
        <MediaIcon size={36} className="text-[color:var(--muted)] group-hover:text-[color:var(--accent)] transition" />
      ...
      <div className="p-4">
        ...
        {b.trackCount != null ? (
          <p className="mt-0.5 text-xs text-[color:var(--muted)]">
            {b.trackCount} {b.trackCount > 1 ? itemsNoun : itemNoun}
          </p>
        ) : null}
      </div>
    </>
