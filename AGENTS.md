...
It wraps `scripts/dev.ts`. The per-app scripts still exist (`dev:musiklash` → :3000, `dev:animeklash` → :3001, `dev:demoklash` → :3002, `dev:screenklash` → :3003) but DON'T chain them — they don't kill siblings.
